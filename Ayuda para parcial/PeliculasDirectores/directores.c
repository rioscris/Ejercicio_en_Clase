...
int removerDirector(eDirectores* lista, int len, char nombre[], ePeliculas* listaPeliculas, int lenPeliculas)
{
    int index;
    int i, j;
    int retorno=-1;
    ...
    if(lista!=NULL)
    {
        ...
        for( i=0 ; i<len ; i++)
        {
            if(index>=0)
            {
                for( j=0 ; j<lenPeliculas; j ++)
                {
                    if(strcmp(lista[i].nombre, listaPeliculas[j].nombreDirector)==0)
                    {
                        listaPeliculas[j].isEmpty=1;
                    }
                }
